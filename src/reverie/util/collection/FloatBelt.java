...
public class FloatBelt{
     ...
     * {@linkplain FloatBelt#lockFlag acquisition bit}.
     */
    ...
    public FloatBelt(int initialCapacity){
        head = tail = new Segment();
        head.data = new float[initialCapacity];
        ...
        while(true){
            ...
            if(data.length >= newLength - precedingLength){
                ...
                try{
                    ...
                    next = new Segment();
                    next.data = new float[Math.max(length, count)];
                }catch(Throwable t){
    ...
    public interface Enqueuer{
        void get(float[] dst, int offset);
    }

    public interface Clearer<T>{
        T get(float[] dst, int length);
    }
